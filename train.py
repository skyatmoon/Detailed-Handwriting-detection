...
from IAMloade import dataset, loader_param, words_rep
...
parser.add_argument('--max_epoch', default=1, type=int, help="max training epoch")
...
parser.add_argument('--pre_train', default=False, type=bool, help="whether use pre-train weights")
...
def eng():
    ...
    return eng_alpha2index


def val(net, valSet, ctc_loss, max_iter=100):
    net.eval()
    data_loader = DataLoader(valSet, args.batch_size, shuffle=True, num_workers=args.num_workers)
    ...
    for i in range(max_iter):
        ...
        labels, target_lengths = words_rep(labels, max_out_chars = 20, batch_size = args.batch_size, eng_alpha2index = eng_alpha2index)
        i += 1
        preds = net(images)
...
def train(net, optimizer, trainSet, valSet, use_gpu, input_lengths):
    ...
    epoch_size = math.ceil(len(trainSet) / args.batch_size - 1)
    ...
    for iteration in range(start_iter, max_iter):
        ...
        labels, target_lengths = words_rep(labels, max_out_chars = 100, batch_size = args.batch_size, eng_alpha2index = eng_alpha2index)
        # T = 50
        # target_lengths = torch.zeros((args.batch_size,), dtype=torch.int)
        # input_lengths = torch.zeros((args.batch_size,), dtype=torch.int)
        labels = labels.long()
        print(labels.shape)
        if use_gpu:
            images = images.cuda()
...
if __name__ == '__main__':
    ...
    if args.pre_train:
        pretrained_dict = torch.load(os.path.join(args.weights_save_folder, "Final-crnn.pth"))
    ...
    val_set = dataset(image_root="/home/skyatmoon/COMP4550/IAM/test/", label_root = "/home/skyatmoon/COMP4550/IAM/test.txt", img_x = img_x, img_y = img_y)
    val_loader = torch.utils.data.DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=0)
    input_lengths = torch.full(size=(args.batch_size,), fill_value=args.batch_size, dtype=torch.long)


    
    #train(net, optimizer, trainSet, valSet, use_gpu)
    train(net, optimizer, train_set, val_set, use_gpu, input_lengths)
