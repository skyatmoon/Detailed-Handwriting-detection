...
parser.add_argument('-m', '--trained_model', default='./CRNN-model/Final-crnn.pth',
                    type=str, help='Trained state_dict file path to open')
parser.add_argument('--save_file', default='result.txt', type=str, help='file to save results')
parser.add_argument('--width', default=500, type=int, help="input image width")
...
parser.add_argument('--input_path', default='./words-level/', type=str, help="image or images dir")
...
def img_cro(img,pieces,points,name,num):

    for i in range(len(points)-1):
        cropped = img[0:int(img.shape[0]), (int((img.shape[1]/pieces)*points[i])): (int((img.shape[1]/pieces)*points[i+1]))] # 裁剪坐标为[y0:y1, x0:x1]
        #img = cv2.line(img, pt1=(int(img.shape[0]), 0), pt2=(int(img.shape[0]),(int((img.shape[1]/pieces)*points[i+1]))), color=(0, 0, 255), thickness=10)
    # cv2.imshow('aaa', img)
    # cv2.waitKey(0)
    #cv2.imwrite("./res/"+ str(name) +  ".png", img)
        cv2.imwrite("./character-level/"+ str(name[i]) + "_" + str(num) + ".png", cropped)
...
if __name__ == '__main__':
    ...
    for img_path in image_paths:
        ...
        print(cut_points)
        print(lab2str)
        img_cro(image,length,cut_points,lab2str, num)
        end = time.time()
        num = num + 1
