...
def custom_collate_fn(batch, T=75):
    # T 根据设计的网络输出来
...
def process_img(img, height, width, transform):
    ...
    if (img_size[1] / (img_size[0] * 1.0)) < 6.4:
        img_reshape = cv2.resize(img, (int(32.0 / img_size[0] * img_size[1]), height))
...
def decode_out(str_index, characters):
    ...
    for i in range(len(str_index)):
        if str_index[i] != 0:
            length = i +1
            if(not (str_index[i + 1] == str_index[i])):
                char_list.append(characters[str_index[i]])
                points.append(i)
        
    return ''.join(char_list),points, length
