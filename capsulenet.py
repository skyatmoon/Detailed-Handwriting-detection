...
import os.path
import glob
from datapro import LocalDataset, DataPreprocess
...
def draw_mask_edge_on_image_cv2(image, mask, color=(0, 0, 255)):
    ...
    cv2.imwrite(args.save_dir + "/real_and_recon_test.png", image)
...
def test(model, test_loader, args):
    ...
    for x, y in test_loader:
        y = torch.zeros(y.size(0), 63).scatter_(1, y.view(-1, 1), 1.)
...
def train(model, train_loader, test_loader, args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (x, y) in enumerate(train_loader):  # batch training
            y = torch.zeros(y.size(0), 63).scatter_(1, y.view(-1, 1), 1.)  # change to one-hot coding
...
def load_emnist(path='./data', download=False, batch_size=100, shift_pixels=2):
    """
    Construct dataloaders for training and test data. Data augmentation is also done here.
    :param path: file path of the dataset
    :param download: whether to download the original data
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        EMNIST(path,split="letters",train=True, download=download,
                       transform=transforms.Compose([transforms.RandomCrop(size=28, padding=shift_pixels),
                                                     transforms.ToTensor()])),
        batch_size=batch_size,shuffle=True, **kwargs)

        #EMNIST can autoly downloade by rewrite the EMNIST() function by myself, but the training still cannot running OMG

    test_loader = torch.utils.data.DataLoader(
    
        EMNIST(path,split="letters",train=False, download=download,
                       transform=transforms.ToTensor()),
        batch_size=batch_size, shuffle=True, **kwargs)
...
def load_mymnist(path='./littlemnist/', batch_size=2):

    train_data = LocalDataset(path + 'train.txt')
    test_data = LocalDataset(path + 'test.txt')
...
"""
training can no worry running "python capsulenet.py", will train on MNIST in 5 epochs (only neeed 5min, faster than downloade a pretrained one)
, acc will be 0% but the reconstruction can really good

show reconstruction can use "python capsulenet.py --testing --weights result/trained_model.pkl" when you have an trained model
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', default='./littlemnist/',
    ...
    # define model
    model = CapsuleNet(input_size=[1, 28, 28], classes=63, routings=3)
    model.cuda()
    print(model)
